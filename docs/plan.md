...
# TDD Plan: ValidationService with Sealed Class Result

## Feature Description
Add a type-safe validation system that wraps Jakarta Bean Validation with a sealed class result type. This provides compile-time guarantees about handling validation results and groups violations by property path.

## Implementation Approach
- Create `ValidationResult<T>` sealed class with `Valid` and `Invalid` variants
- Implement `ValidationService` that wraps the Jakarta validator
- Group validation violations by property path for easier rendering
- Return strongly-typed results that force proper error handling

## Benefits
- Type-safe validation result handling (compiler enforces checking both Valid and Invalid cases)
- Clean API for validation logic
- Violations grouped by field name for easy form rendering
- Demonstrates advanced Kotlin patterns (sealed classes, generics)
...
**Test**:
- `ValidationResult.Valid` should hold a value
- `ValidationResult.Invalid` should hold a map of property paths to error messages
...
sealed class ValidationResult<T> {
    ...
    data class Invalid<T>(val violations: Map<String, List<String>>) : ValidationResult<T>()
}
```
...
**Implementation**:
- Create `ValidationService` class with constructor parameter `Validator`
- Implement `validate<T>(value: T): ValidationResult<T>` method
...
**Implementation**:
- In `validate()`, if violations are not empty:
  - Group violations by `it.propertyPath.toString()`
...
Example: A field with `@NotBlank` and `@Size(max=10)` failing both validations should have a list of 2 messages.
...
## Notes
- ValidationService is a thin wrapper around Jakarta Validator
- The sealed class forces exhaustive when-expression handling at call sites
- This pattern works well with HTMX partial updates - can return specific field errors
- Consider adding extension functions for common patterns (e.g., `getOrThrow()`, `mapValid()`)
