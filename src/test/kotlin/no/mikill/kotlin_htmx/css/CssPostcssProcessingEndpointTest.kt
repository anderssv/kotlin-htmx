...
class CssPostcssProcessingEndpointTest {
    @Test
    fun `should process CSS with autoprefixer and return prefixed CSS`() =
        testApplication {
            application {
                module()
            }
            ...
            assertTrue(
                responseBody.contains("transform:") || responseBody.contains("-webkit-transform:"),
                "Should contain transform property with potential vendor prefixes",
            )

            // Test that SCSS variables were processed
            assertTrue(
                responseBody.contains("#007bff") || responseBody.contains("background-color: #007bff"),
                "Should process SCSS variables",
            )

            // Test that calc expressions were processed
            assertTrue(
                responseBody.contains("calc(") || responseBody.contains("8px"),
                "Should process calc expressions",
            )
        }

    @Test
    fun `should process CSS with all PostCSS plugins using existing test file`() =
        testApplication {
            application {
                module()
            }
            ...
            assertTrue(
                responseBody.contains("8px") || responseBody.contains("calc("),
                "Should process calc expressions",
            )
            ...
            assertTrue(
                responseBody.contains("transform:") || responseBody.contains("transform "),
                "Should contain transform property",
            )
        }

    @Test
    fun `should return 404 for non-existent CSS file`() =
        testApplication {
            application {
                module()
            }
